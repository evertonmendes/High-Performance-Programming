...
void mat_mult_AdjList(struct Graph *graph, double *vector, int n_vertexs, double *new_vector);
...
int main(int argc, char *argv[])
{
    ...
    double verify_norm = normalize_vec(file_data->n_vertexs, vec);
    ...
    while (stop_iter < 3)
    {
        ...
        mat_mult_AdjList(file_data->graph, vec, file_data->n_vertexs, new_vec);
...
void mat_mult_AdjList(struct Graph *graph, double *vector, int n_vertexs, double *new_vector)
{
    ...
    for (int i = 0; i < n_vertexs; i++)
    {
        ...
        else
        {
            while (ptr != NULL)
            {
                new_vector[i] += (ptr->weight) * (vector[ptr->dest]);
