...
void returnNumberNeighbors(struct Graph *graph, int n_vertexs, int *n_neighbors)
{
    for (int i = 0; i < n_vertexs; i++)
    {
        struct Node *ptr = graph->head[i];
...
void neighborsWeight(struct Graph *graph, int vertex, int n_neighbors, double *weights)
{
